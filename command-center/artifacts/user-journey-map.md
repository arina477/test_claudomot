...
last_updated: 2026-06-30 (T-9 wave-14 — M3 PRESENCE layer LIVE; NEW /presence Socket.IO namespace (online/offline ref-count, snapshot-on-join, multi-tab no-flap), typing indicators (channel-scoped), and the page-9 member-list panel (grouped Online/Offline, avatar+name+presence-dot, responsive collapse <1024). NEW REST: GET /servers/:id/members (member-gated 401/403). Membership-scoping LOAD-BEARING-PROVEN with TWO DISTINCT VERIFIED users (fixture A + provisioned account-b B, co-members): B sees presence:online{A} 311ms / offline{A} 79ms; NO-LEAK (0 foreign ids; typing channel-scoped); WS rejects unauth+unverified. **KNOWN DEFECT F-4 (HIGH → V-2): typing indicator structurally non-functional — emitTypingActive excludes the actor from ALL recipients, so co-members never see "<name> is typing"; task 58633934 core AC unmet in prod.** No new page route — extends page-9. Author-row presence dots deferred (10b9d18e). Verified-prod fixtures: studyhall-e2e-fixture + studyhall-e2e-fixture-b)
version: 0.10
status_legend:
