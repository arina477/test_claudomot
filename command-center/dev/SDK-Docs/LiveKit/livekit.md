...
**Wave-34: screen-share grant widening**

`canPublishSources` supersedes `canPublish` when set. Adding `TrackSource.SCREEN_SHARE` and `TrackSource.SCREEN_SHARE_AUDIO` to the array is required for any client `setScreenShareEnabled(true)` to succeed server-side. Without them, the server rejects the publish even when `canPublish: true` is also set. Camera is excluded by omitting `TrackSource.CAMERA`.

```typescript
// voice-token.service.ts (wave-34 grant)
at.addGrant({
  roomJoin: true,
  room: channelId,
  canPublish: true,
...
**Wave-34: ConnectionQuality + subscription control for audio-only fallback**

`RoomEvent.ConnectionQualityChanged` fires with `(quality: ConnectionQuality, participant: Participant)`. Filter to `room.localParticipant` to track local-only quality. `ConnectionQuality.Poor` and `ConnectionQuality.Lost` are the degrade triggers; `ConnectionQuality.Good` / `ConnectionQuality.Excellent` are the recover triggers.

`RemoteTrackPublication.setSubscribed(false)` pauses inbound video/screen-share tracks without dropping audio. `setSubscribed(true)` re-enables them. This is the correct subscription-control surface for client-side bandwidth management — do not call `track.stop()` or `track.disable()` which affect the local publish side.

```typescript
// Pause all remote video (keep audio — the invariant):
...
Debounce `ConnectionQuality.Poor` events (3 s recommended) before entering audio-only mode to avoid subscription thrash during transient quality dips.
