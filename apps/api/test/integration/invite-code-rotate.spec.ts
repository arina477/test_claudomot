...
describe.skipIf(SKIP)('ServersService.rotateInviteCode — real-Postgres (task d058283d)', () => {
  ...
  beforeEach(async () => {
    ...
    await harnessQuery(`UPDATE servers SET invite_code = 'initial-perm-code-aaa' WHERE id = $1`, [
      SERVER_ID,
    ]);
  });
  ...
  it('throws NotFoundException (404) for a non-existent serverId (AC5)', async () => {
    await expect(sut.rotateInviteCode(NONEXISTENT_SERVER_ID, OWNER_ID)).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
});
