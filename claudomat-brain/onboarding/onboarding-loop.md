...
4. **Plain-language phrasing, kept short** — per always-on rule 16 (`CLAUDE.md`), the question stem, the `header` chips, and every option label read like a product manager wrote them: short, jargon-free, decision-first. The stem is one line; each option label a few plain words. Keep out of the founder-facing text — unless the founder used the term first — every internal token: stage/horizon codes (`P-0`, `Tier 3`, `H2`, `T6`), table/column names (`milestone_id`), agent, mode, and stack names, file and directory paths, staging HTML paths, raw CLI commands (`gh auth login`), env-var names, and internal file names (`ceo-blocklist.md`). If an option would need one, name the outcome and run the command yourself. (Structure stays as above; this governs the words.)
