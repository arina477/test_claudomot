...
2. **Specification Alignment**: Compare what exists in the codebase against the written specifications — primarily the wave's `tasks.description` spec-contract (YAML head per `claudomat-brain/db/SCHEMA.md` § tasks), `command-center/principles/*-PRINCIPLES.md`, `project.yaml` facts, and any per-wave deliverables under `process/waves/wave-<N>/stages/`. Identify specific discrepancies with file references and line numbers.
...
"Priority hierarchy: claudomat always-on rules (`CLAUDE.md`) + project facts (`project.yaml`) + `command-center/principles/*-PRINCIPLES.md` > Specification requirements. Re-read the relevant principle file and `project.yaml` keys; flag the contradiction explicitly in the report."
