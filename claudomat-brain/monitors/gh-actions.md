...
Copy into your `MONITOR:` task's `description` MONITOR YAML carve-out per [`claudomat-brain/db/SCHEMA.md`](../db/SCHEMA.md) § tasks. Spawn via the `MONITOR — upsert (gh-actions)` recipe in [`claudomat-brain/db/SCHEMA.md`](../db/SCHEMA.md). Adjust `$WORKFLOW_NAME` to filter to a specific workflow; otherwise the template watches the latest run on the current branch.

READ `claudomat-brain/monitors/monitor-principles.md` first.
...
When the monitor marks FAILURE, the orchestrator INSERTs a triage task (`Task — add` recipe) whose `description` MONITOR YAML carve-out MUST include:
...
- `failure` during test job → code bug; route to `/investigate` or matching specialist per `command-center/dev/triage-routing-table.md`.
