...
Copy into your `MONITOR:` task's `description` MONITOR YAML carve-out per [`claudomat-brain/db/SCHEMA.md`](../db/SCHEMA.md) § tasks. Spawn via the `MONITOR — upsert (netlify)` recipe in [`claudomat-brain/db/SCHEMA.md`](../db/SCHEMA.md). Set `$NETLIFY_SITE_ID` (from `netlify sites:list --json`).
