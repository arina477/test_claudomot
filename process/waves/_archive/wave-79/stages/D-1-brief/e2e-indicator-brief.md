...
- **Colors:**
  ...
  - `--text-muted` (`rgba(255,255,255,0.40)`) — the de-emphasized **undecryptable-payload monospace shell** only. **NOT for status labels:** the **cannot-decrypt-on-this-device** and **loading/establishing** STATUS LABELS use `--text-secondary` (`rgba(255,255,255,0.60)`), because `--text-muted` computes below WCAG AA 4.5:1 on the badge surfaces (DESIGN-PRINCIPLES rule 1). **[D-3 attempt-2 correction S2 — B-3 must use `--text-secondary` for these labels, not `--text-muted`.]**
...
- **Typography (§ 2):** `text-xs` (12px, medium 500) for badge label + per-message micro-label (matches ConnectionStateIndicator pill + "Sending…"/"Failed" labels); tooltip/popover body `12px` per DESIGN-SYSTEM § 8 Tooltip primitive (canonical — supersedes an earlier `text-sm`/14px note here; DESIGN-SYSTEM is the token source of truth); Geist family; tooltip body line-height 1.5. **[D-3 attempt-2 correction S1]**
