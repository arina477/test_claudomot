...
## P-4 PHASE-2 ANNOTATIONS (carry to B-3)
- **Gemini→head-product MATERIAL (conditional last_reply_at recompute):** in deleteReply's soft-delete txn — `reply_count` ALWAYS decrements; `last_reply_at` recomputes `MAX(created_at)` over remaining live replies ONLY when the deleted reply was the tail (`deletedReply.created_at == parent.last_reply_at`); otherwise leave last_reply_at unchanged. NULL when no live replies remain. (Makes the invariant "last_reply_at only moves when the tail moves" explicit; avoids a wasted MAX on non-tail deletes.) No new index/trigger.
- **karen B-3 carry (txn template):** createMessage is NOT transactional (sequential standalone awaits — messages.service.ts:255-317). Use `createServer`'s `db.transaction(async (tx) => …)` pattern (servers.service.ts:68 — a proven multi-table-insert txn) as the TEMPLATE for createReply's atomic (insert reply + reply_count++/last_reply_at) boundary AND deleteReply's (soft-delete + count--/recompute). Do NOT copy createMessage's non-transactional shape for the atomicity boundary.
