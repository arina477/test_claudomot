# Wave 15 — D-block review artifacts

**Block:** D (Design)
**Wave topic:** M3 @mentions UI — composer autocomplete dropdown + mention-pill + unread-mention affordance
**Block exit gate:** D-3
**Status:** in-progress

## Stage deliverables
| Stage | Deliverable file(s) | Status | Notes |
|---|---|---|---|
| D-1 | stages/D-1-brief/{mention-autocomplete,mention-pill,unread-mention}-brief.md | done | 3 briefs, mask PASS |
| D-2 | stages/D-2-variants/*-variants.md + *-iterate.md | pending | /aidesigner compose onto server-channel-view |
| D-3 | stages/D-3-review-and-adopt/* | pending | dual-reviewer + head-designer gate + canonicalize |

## Block-specific context
- **Wave topic:** @-autocomplete dropdown (composer), inline mention-pill (message rows, viewer-targeted distinct), unread-mention badge/affordance
- **design_gap_flag:** true (from P-1)
- **Gaps inventoried:** [mention-autocomplete, mention-pill, unread-mention] (D-1 Action 1)
- **All 3 COMPOSE onto design/server-channel-view.html** (canonical, has composer + message rows + member-list + reaction-pill prior art)
- **DESIGN-PRINCIPLES rule 1 (wave-14):** muted text on dark surfaces ≥4.5:1 by calculation — applies to mention-pill + unread treatments.

## Open escalations carried into gate
none

## Gate verdict log
<appended by head-designer at D-3>
