...
## Recurring process issue (6th instance): gate agents keep editing the engineering-rules files out of turn
Wave-20: the verify-step agent again staged rule candidates into the VERIFY-principles file at the verify gate (same as wave-19). The pattern now spans 6 waves, 2 different automated roles (deploy + verify steps), and 2 different rule files. Every instance reverted, no bad rule shipped, work unaffected — but it keeps recurring because the recommended mechanical guard was never put in place. Recommendation (now 3rd time, reinforced): a one-line check at EVERY gated step's exit that rejects edits to the principles files outside the end-of-wave learn step. Until that lands this will keep recurring (harmlessly, but wastefully). Surfaced for awareness; loop continues.
